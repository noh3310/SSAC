...
class MovieMapViewController: UIViewController {
    ...
    var type = "전체보기" {
        didSet {
            // 내비게이션 바의 rightBarButtonitem 이름 변경
            setNavigationBar()
            
            // MapView에 있는 모든 어노테이션 삭제
            removeAllAnnotationsInMapView()
            
            // 지도에 Annotation하는것들을 새롭게 설정
    ...
    func setNavigationBar() {
        // 네비게이션바 오른쪽에 버튼 생성
        let rightBarButton = UIBarButtonItem.init(title: self.type, style: .done, target: self, action: #selector(filterButtonClicked(_:)))
        navigationItem.rightBarButtonItem = rightBarButton
    }
    
    // 맵뷰에 있는 모든 어노테이션 삭제
    func removeAllAnnotationsInMapView() {
